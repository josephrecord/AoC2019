...
for wire in p1:
    direction = wire[0]
    dist = int(wire[-(len(wire) - 1):])
    if direction == "R":
        xpos = current_pos[0]
        ypos = current_pos[1]
        for x in range(xpos + 1, xpos + dist + 1):
            time += 1
            current_pos = (x, ypos)
            coordinates1[current_pos] = time
    elif direction == "L":
        xpos = current_pos[0]
        ypos = current_pos[1]
        for x in range(xpos - 1, xpos - dist - 1, -1):
            time += 1
            current_pos = (x, ypos)
            coordinates1[current_pos] = time
    elif direction == "U":
        xpos = current_pos[0]
        ypos = current_pos[1]
        for y in range(ypos + 1, ypos + dist + 1):
            time += 1
            current_pos = (xpos, y)
            coordinates1[current_pos] = time
    elif direction == "D":
        xpos = current_pos[0]
        ypos = current_pos[1]
        for y in range(ypos - 1, ypos - dist - 1, -1):
            time += 1
            current_pos = (xpos, y)
            coordinates1[current_pos] = time
    else:
...
for each in intersection:
    k = coordinates1.get(each)
    k2 = coordinates2.get(each)
    times.append(k+k2)
    #print("intersection: ", each)
    #print("time for path 1: ", k)
    #print("time for path 2: ", k2)
